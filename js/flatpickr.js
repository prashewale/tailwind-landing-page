...
***************************************************************************** */
					...
					for (
						...
						ops.forEach(function (_a) {
							var fn = _a.fn,
								val = _a.val;
							return (parsedDate =
								fn(parsedDate, val, l10n) || parsedDate);
						});
					}
	...
	function FlatpickrInstance(element, instanceConfig) {
		...
		function init() {
			...
			if (
				...
				self.calendarContainer.style.width =
					self.daysContainer.offsetWidth +
		...
		function bindEvents() {
			...
			bind(window.document.body, 'focus', documentClick, {
				capture: true
			});
		...
		function animateDays(e) {
			if (
				self.daysContainer &&
				self.daysContainer.childNodes.length > 1
			) {
				switch (e.animationName) {
					case 'fpSlideLeft':
						self.daysContainer.lastChild &&
							self.daysContainer.lastChild.classList.remove(
								'slideLeftNew'
							);
					...
					case 'fpSlideRight':
						self.daysContainer.firstChild &&
							self.daysContainer.firstChild.classList.remove(
								'slideRightNew'
							);
						self.daysContainer.removeChild(
							self.daysContainer.lastChild
		...
		function animateMonths(e) {
			switch (e.animationName) {
				...
				case 'fpSlideRightNew':
					self.navigationCurrentMonth.classList.remove(
						'slideLeftNew'
					);
					self.navigationCurrentMonth.classList.remove(
						'slideRightNew'
			...
			Object.defineProperty(self, '_hidePrevMonthArrow', {
				...
				set: function (bool) {
					if (self.__hidePrevMonthArrow !== bool)
						self.prevMonthNav.style.display = bool
							? 'none'
							: 'block';
			...
			Object.defineProperty(self, '_hideNextMonthArrow', {
				...
				set: function (bool) {
					if (self.__hideNextMonthArrow !== bool)
						self.nextMonthNav.style.display = bool
							? 'none'
							: 'block';
		...
		function buildTime() {
			...
			self.hourElement.title = self.minuteElement.title =
				self.l10n.scrollTitle;
		...
		function onMouseOver(elem) {
			...
			var _loop_1 = function (i, date) {
				...
				var minRangeDate = Math.max(
					...
					maxRangeDate = Math.min(
						self.maxRangeDate.getTime(),
						rangeEndDate
					);
				elem.classList.add(
					hoverDate < self.selectedDates[0]
						? 'startRange'
						: 'endRange'
				);
				if (
		...
		function minMaxDateSetter(type) {
			return function (date) {
				...
				if (self.selectedDates) {
					self.selectedDates = self.selectedDates.filter(function (
						d
					) {
						return isEnabled(d);
					});
			...
			var min = parseFloat(input.getAttribute('data-min')),
				...
				delta =
					...
					(isKeyDown
						...
						: Math.max(
								-1,
								Math.min(1, e.wheelDelta || -e.deltaY)
						  ) || 0);
